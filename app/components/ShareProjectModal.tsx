...
interface ShareProjectModalProps {
  ...
  location: string;
  /** Project.status — when not "ACTIVE" the project isn't publicly visible yet
   *  (e.g. "PENDING_APPROVAL" awaits admin review), so the link isn't live. */
  status?: string;
}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl max-w-md w-full p-6 shadow-2xl relative space-y-6 text-center text-white">
        ...
        <div className="flex flex-col items-center space-y-2">
          ...
          <h3 className="text-xl font-black tracking-tight text-white mt-2">
            {isLive ? "Your project is live!" : "Submitted for review"}
          </h3>
          <p className="text-xs text-gray-400 max-w-sm">
            {isLive
              ? "Spread the word and invite supporters to trace and verify every contribution in real-time."
              : "An admin needs to review and approve your project before it's publicly visible and donatable. You'll be notified once it's live — then you can share it."}
          </p>
        </div>
