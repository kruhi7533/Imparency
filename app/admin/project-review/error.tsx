...
export default function AdminProjectReviewError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error("Admin project review error caught:", error);
  }, [error]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col justify-center items-center px-4 font-sans transition-colors duration-200">
      <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-8 max-w-md w-full shadow-lg text-center space-y-6 animate-in fade-in duration-200">
        <div className="w-16 h-16 bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 rounded-full flex items-center justify-center text-3xl mx-auto shadow-inner">
          ⚠️
        </div>
        <div className="space-y-2">
          <h2 className="text-xl font-black text-gray-900 dark:text-white">Project Review Error</h2>
          <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
            An error occurred loading projects awaiting approval or the review audit trail.
          </p>
        </div>
