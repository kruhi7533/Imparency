...
| **Add a database field** | Edit `prisma/schema.prisma` → `npx prisma db push` (no Prisma Migrate in this repo — see `.env.example`'s per-branch-Neon-database note) → regenerate client (`prisma generate`, also runs via `predev`). Update every route/page that does an explicit `select`/serialization for that model (Decimal/Date fields need manual `Number()`/`.toISOString()` at each call site — no shared serializer). |
