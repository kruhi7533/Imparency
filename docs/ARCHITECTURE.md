...
- **Console IA:** navigation is declared in one place, `app/admin/components/hubs.ts` (`ADMIN_HUBS` + `hubForPath`), rendered by `AdminNav` (top level) and `AdminTabs` (second level). A route claimed by no hub renders no tab bar — that is how a surface is retired from the console without deleting it.
- **The funder-led track is no longer an admin surface.** Opportunities, matching runs and proposals left the console: the `grants` hub is gone from `ADMIN_HUBS`, and three `InboxSources` (`submittedOpportunities`, `openProposals`, `proposedCandidates`) were removed from `lib/today-inbox.ts`/`lib/today-sources.ts` along with their `opportunity`/`proposal`/`candidate` icon keys. Matching a CSR requirement to an NGO is those two parties talking, not an admin decision — it moves to the CSR and NGO panels (see `docs/WEEK5-FLOW.md` for the target lifecycle and the build order). **Nothing was deleted:** `app/admin/opportunities/*`, `app/admin/proposals/*`, `app/api/admin/matching/*`, `app/api/admin/proposals/*` and `lib/matching/*` all still exist and still work, the funder gate in `lib/matching/funder.ts` is unchanged, and the audit trail still deep-links to opportunity detail (`app/admin/audit/page.tsx`). They are simply unreachable from the nav. Admin keeps NGO and CSR verification, and is intended to re-enter this flow only for the cross-border exception — which is not yet detectable, because the funder side has no jurisdiction field.
...
### Donor Organisation Verification *(ADM-003)*
- **Purpose:** verify the **company** behind a corporate donor, as distinct from the **person** holding the account. `panStatus` answers "is this human's tax id real"; `orgVerificationStatus` answers "is the company real" — the question an NGO is implicitly trusting when it receives a shortlist email. Before this, only the first question had an answer, and `lib/matching/funder.ts` said so in its header comment.
- **Files:** `lib/csr-verification.ts` (pure rules, no model call); `app/api/admin/donors/[id]/org-review/route.ts` (the only writer of `VERIFIED`); `app/api/donor/profile/route.ts` (submission + retirement only); `app/admin/donors/[id]/OrgVerificationActions.tsx`; queue filter on `app/admin/donors/page.tsx`.
- **DB models:** `User.cin`, `User.orgVerificationStatus` (`OrgVerificationStatus`), `orgSubmittedAt`, `orgVerifiedAt`, `orgVerifiedById`, `orgVerificationNote`.
- **Rules worth not breaking:**
  - `NOT_SUBMITTED` is the default and carries weight — individual donors must never appear in the queue. Reusing `VerificationStatus` would have defaulted all of them to `PENDING`.
  - A donor's own profile save can reach `PENDING` but **never** `VERIFIED` — `decideOrgStatus()` has no path to it. Same separation as `ExtractedField`, where only a human PATCH produces `VALIDATED`.
  - Completeness is re-checked at decision time, not trusted from the `PENDING` status, because the profile can have been edited since.
  - Editing `companyName` or `cin` after approval retires the approval (`PENDING` + `CSR_ORG_REOPENED`). A verified badge must not survive an entity swap.
- **The gate:** `checkFunderEligibility` (`lib/matching/funder.ts`) now requires **both** `panStatus === VERIFIED` (the person) and `orgVerificationStatus === VERIFIED` (the body). It runs at opportunity **create** and at the **OPEN** transition only — already-OPEN opportunities are not re-checked, so enabling this closed nothing that was already live. The two failures are reported one at a time, because different people fix them.
- **One gate, three kinds of evidence.** The gate is uniform — VERIFIED or you do not fund, the same standard `lib/matching/runner.ts` holds NGOs to. What *earns* it depends on `orgKindFor(donorPersona)`: a `COMPANY` needs a valid CIN and CSR budget, a `TRUST` needs its registration number and grant budget, a `GOVERNMENT` body needs only a name (no registry exists to check a department against, so the admin's judgement **is** the check and the gate records that they made it). Demanding a CIN of everyone would have locked every foundation out permanently — a gate nobody can pass rather than a gate that works.
- **`companyName` is the legal name of any funding body**, not just a company. It used to be nulled unless `isCorporate`, which left a trust with nowhere to record its own name.
- **The CSR certificate is gated too.** `app/api/donations/csr-certificate/route.tsx` used to issue on the self-declared `isCorporate` checkbox; it now requires `orgVerificationStatus === VERIFIED` and prints the CIN on the document. That PDF names a "Corporate Entity", states a utilisation figure and signs itself "ImpactBridge Compliance — Authorized Verifier", so verification is what earns the signature.
- **The donation path is gated too, and this was the bigger hole.** `app/api/donations/create-order/route.ts` checked session, role, rate limit, project-ACTIVE and FCRA — never the organisation. So being *named* behind an opportunity was gated while *paying* was not, which is the wrong way round: the payment is the part that moves money, and the Donate button walked around the whole funding rail. The gate is scoped to `isFunderPersona(donorPersona)`, so individual and HNI giving (and every donor with no persona set — most existing rows) is untouched.
- **Admin visibility.** Two new `InboxSources` feed the Action Center: `pendingOrgs` (Waiting on you — a company awaiting a decision, the donor-side twin of `pendingNgos`) and `institutionalDonations` (Signals — corporate money in the last 30 days, HIGH when the organisation behind it was never verified, LOW when it was). The second is a signal and never a queue item: the payment already completed, so there is nothing to decide.
- **The declared budget now does something** (`lib/csr-budget.ts`, pure). `assessBudgetUtilisation()` bands a donor's giving for the CURRENT financial year against the budget they declared: `WITHIN` / `NEAR` (≥90%) / `OVER` / `UNDECLARED`. Surfaced on the donor 360 page and used by `checkCsrBudgetOverrun` in `lib/risk-agent.ts`.
  - **It never blocks a donation, and must not start to.** Exceeding a declared CSR budget is not a breach — §135 of the Companies Act obliges a company to spend *at least* 2% of net profit, so over-spending is lawful and common. Over-utilisation is a data-quality signal (the declared figure is stale) or a risk signal, and the alert text says so explicitly so nobody reads it as fraud.
  - **`UNDECLARED` is not `WITHIN`.** A donor with no declared budget renders as "no annual budget declared", never as within budget.
  - **Three bugs fixed in `checkCsrBudgetOverrun`:** it compared LIFETIME `totalDonated` against an ANNUAL figure (and documented that it was not like-for-like); it used a 2× multiplier, so a donor could give double their declared budget before anything fired; and it returned early for any persona other than `CSR_OFFICER`, exempting every FOUNDATION from a check it appeared covered by. `financialYearRange()` in `lib/finance-utils.ts` is the half-open [1 Apr, 1 Apr) window it now uses. Assertions in `tests/donor-rule-checks.test.ts` were updated deliberately — they pinned the old behaviour.
- **The certificate's copy was making two false claims, now fixed.** The footer claimed "Verification hashes can be tracked on-chain" — there is no blockchain anywhere in this codebase — and the declaration box asserted that milestone completion "ha[d] been independently validated" as *static text*, printed unchanged above a table showing 0 milestones and an N/A score. The declaration is now derived from the same `milestonesCompleted` rows the table renders, so the two cannot disagree; with none approved it says the certificate "records the contribution only — not delivery or outcomes".

### Payment mode — absence of config is never permission
`lib/payment-mode.ts` (`resolvePaymentMode`) decides whether a request may take a **mock** payment. Both `app/api/donations/create-order/route.ts` and `app/api/crisis/[id]/donate/route.ts` used to decide inline with `!process.env.RAZORPAY_KEY_ID || …placeholder…`, which **fails open**.

That matters because mock mode is not "simulate and stop": it writes the donation `SUCCESS`, increments `raisedAmount` and `totalDonated`, moves milestones to `IN_PROGRESS`, and **issues an 80G tax receipt** — all of it inline, bypassing the idempotent webhook entirely. A production deploy that lost `RAZORPAY_KEY_ID` (rotated secret, typo, new environment) would therefore have started minting tax receipts for money nobody paid, silently. Now: `LIVE` when credentials exist, `MOCK` only outside production, and `MISCONFIGURED` in production — which both routes turn into a 503 rather than a fallback. This also explains stray `TaxReceipt` rows for donations with no webhook delivery: mock mode created them.

### Test environment hygiene
`vitest.config.ts` sets `fileParallelism: false`, so every test file shares one process and one `process.env`. `tests/fraud-investigator-openai-loop.test.ts` set six `INVESTIGATOR_*` variables and cleaned up none, and `tests/fraud-investigator-resolve-ngo.test.ts` was passing **only** because it inherited them — it needs an enabled investigator to reach the guard it tests, and set none of its own. Both are now self-contained. When adding a test that sets env, delete it in `afterEach`.
- **Fixtures:** `npm run seed:csr` seeds the acceptance-pack donors (Suryodaya complete, Halfway missing a CIN, one individual).
